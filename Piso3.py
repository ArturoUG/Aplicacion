...
def Piso3():
    ...
    miFrame.geometry("1200x550")
    #-------------Etiquetas

    Label(miFrame, text="ID: ", bg="green").place(x=20, y=20, relx=0.10, rely=0.10)
    Label(miFrame, text="Nombre: ", bg="green").place(x=20, y=40, relx=0.10, rely=0.20)
    Label(miFrame, text="Telefono: ", bg="green").place(x=20, y=60, relx=0.10, rely=0.30)
    Label(miFrame, text="Correo: ", bg="green").place(x=20, y=80, relx=0.10, rely=0.40)
...
def Create():
    ...
    miCursor.execute("INSERT INTO PISO3 VALUES(NULL,'" + miName.get()+ 
    "','" + miTelefono.get() +
    "','" + miCorreo.get() + "')")
...
def Read():
    miConexion = sqlite3.connect("Piso3")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO3 WHERE ID=" + miId.get())
...
def Update():
    miConexion = sqlite3.connect("Piso3")
    miCursor = miConexion.cursor()
    miCursor.execute("UPDATE PISO3 SET NOMBRE= '" + miName.get()+ 
...
def Delete():
    miConexion = sqlite3.connect("Piso3")
    miCursor = miConexion.cursor()
    miCursor.execute("DELETE FROM PISO3 WHERE ID=" + miId.get())
...
def ListarM():
    ...
    miConexion = sqlite3.connect("Piso3")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO3")
    lista = []
