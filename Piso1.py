...
def Piso1():
    ...
    Label(miFrame, text="ID: ", bg="red").place(x=20, y=20, relx=0.10, rely=0.10)
    Label(miFrame, text="Nombre: ", bg="red").place(x=20, y=40, relx=0.10, rely=0.20)
    Label(miFrame, text="Telefono: ", bg="red").place(x=20, y=60, relx=0.10, rely=0.30)
    Label(miFrame, text="Correo: ", bg="red").place(x=20, y=80, relx=0.10, rely=0.40)
...
def Create():
    ...
    miCursor.execute("INSERT INTO PISO1 VALUES(NULL,'" + miName.get()+ 
    "','" + miTelefono.get() +
    "','" + miCorreo.get() + "')")
    miConexion.commit()
...
def Read():
    miConexion = sqlite3.connect("Piso1")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO1 WHERE ID=" + miId.get())
...
def Update():
    miConexion = sqlite3.connect("Piso1")
    miCursor = miConexion.cursor()
    miCursor.execute("UPDATE PISO1 SET NOMBRE= '" + miName.get()+ 
...
def Delete():
    miConexion = sqlite3.connect("Piso1")
    miCursor = miConexion.cursor()
    miCursor.execute("DELETE FROM PISO1 WHERE ID=" + miId.get())
...
def ListarM():
    ...
    miConexion = sqlite3.connect("Piso1")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO1")
    lista = []
