...
def Piso2():
    ...
    Label(miFrame, text="ID: ", bg="gray").place(x=20, y=20, relx=0.10, rely=0.10)
    Label(miFrame, text="Nombre: ", bg="gray").place(x=20, y=40, relx=0.10, rely=0.20)
    Label(miFrame, text="Telefono: ", bg="gray").place(x=20, y=60, relx=0.10, rely=0.30)
    Label(miFrame, text="Correo: ", bg="gray").place(x=20, y=80, relx=0.10, rely=0.40)
...
def Create():
    ...
    miCursor.execute("INSERT INTO PISO2 VALUES(NULL,'" + miName.get()+ 
    "','" + miTelefono.get() +
    "','" + miCorreo.get() + "')")
...
def Read():
    miConexion = sqlite3.connect("Piso2")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO2 WHERE ID=" + miId.get())
...
def Update():
    miConexion = sqlite3.connect("Piso2")
    miCursor = miConexion.cursor()
    miCursor.execute("UPDATE PISO2 SET NOMBRE= '" + miName.get()+ 
...
def Delete():
    miConexion = sqlite3.connect("Piso2")
    miCursor = miConexion.cursor()
    miCursor.execute("DELETE FROM PISO2 WHERE ID=" + miId.get())
...
def ListarM():
    ...
    miConexion = sqlite3.connect("Piso2")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM PISO2")
    lista = []
